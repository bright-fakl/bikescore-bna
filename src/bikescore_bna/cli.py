"""``bikescore-bna`` — the minimal single-city CLI over the core public API.
...
/ ``score_city``) — no workspace, no run store, no database:
    ...
    bikescore-bna score    <city> [--scenario …] [--set k=v …] [--out …]
...
the web CLI all live in bikescore.
"""
...
from bikescore_bna.city import load_city
...
def _resolve_city_dir(city: str) -> Path:
    ...
    _err.print(
        ...
        f"Pass a path to a city directory (slug lookup lives in bikescore)."
    )
    raise typer.Exit(2)
...
def _discover_inputs(datasets_dir: Path) -> dict[str, Path]:
    ...
    if not datasets_dir.is_dir():
        _err.print(
            ...
            f"Run [bold]bikescore-bna acquire[/bold] first, or pass --datasets."
        )
        raise typer.Exit(2)
...
def acquire(
    ...
    pbf_cache_dir: Annotated[
        ...
        typer.Option(
            "--pbf-cache-dir",
            help="Shared regional-PBF cache dir (default: $BIKESCORE_PBF_CACHE or ~/.bikescore-bna/pbf).",
        ),
    ] = None,
    force: Annotated[
...
def scenario_show(
    ...
    from bikescore_bna.scenarios import ScenarioNotFoundError, get_bundled_scenario
...
def export_cmd(
    ...
    """
    from bikescore_bna.export import export_bundle, export_target
...
def main() -> None:
    """Console-script entry point (``bikescore-bna``)."""
    app()
