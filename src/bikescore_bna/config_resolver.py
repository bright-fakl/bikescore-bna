...
def _clear_structural(config: BNAConfig) -> None:
    ...
    """
    from bikescore_bna.attributes import AttributeRegistry
...
def serialize_complete_config(config: BNAConfig) -> dict:
    ...
    from bikescore_bna.decision import Decision
    ...
    for ruleset_name, (ns, field_name) in sorted(_RULESET_LOCATIONS.items()):
        ...
        if isinstance(decision, Decision):
            ...
            from bikescore_bna.decision import decision_to_terse
            if (
    ...
    if config.attributes is not None:
        from bikescore_bna.attributes import DecisionAttribute, PrimaryAttribute
        dsl_attributes = [
...
def _merge_destinations(registry: Any, entries: list) -> None:
    ...
    *full* ``to_dict()`` form emitted by ``serialize_complete_config`` (explicit
    ...
    """
    from bikescore_bna.destinations import (
...
def _merge_intersection_attributes(attrs: list, entries: list) -> list:
    ...
    """
    from bikescore_bna.intersection_attributes import IntersectionAttribute
...
def _apply_layer(
    ...
    for namespace, entries in (layer or {}).items():
        ...
        if namespace == "intersection_attributes":
            ...
            from bikescore_bna.intersection_attributes import default_intersection_attributes
        ...
        if namespace == "attributes":
            ...
            from bikescore_bna.attributes import AttributeRegistry, attribute_from_dict
...
def _to_decision(val: Any) -> Any:
    ...
    """
    from bikescore_bna.decision import Decision, decision_from_doc
...
_RULESET_DEFAULT_LOADERS: dict[str, tuple[str, str]] = {
    "stress_segment": ("bikescore_bna.rules.stress_segment", "default_segment_stress_rules"),
    "stress_intersection": ("bikescore_bna.rules.stress_intersection", "default_intersection_stress_rules"),
}
...
_RULESET_AUTHORED_LOADERS: dict[str, tuple[str, str]] = {
    "stress_segment": ("bikescore_bna.rules.stress_segment", "authored_segment_stress_doc"),
    "stress_intersection": (
        "bikescore_bna.rules.stress_intersection", "authored_intersection_stress_doc"),
}
...
# no city dir, no ``city_config`` layering, no provenance. The app (bikescore)
...
def list_bundled_scenarios() -> list[str]:
    ...
    from bikescore_bna.scenarios import list_bundled_scenarios as _list
