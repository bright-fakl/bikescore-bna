...
through :func:`bikescore.validation.compare_dataframes` with the right row-alignment
strategy and returns a report per stage.

This is a **development / QA** harness (parity against a ground-truth reference), reused
by the parity test suite and the ``bikescore-score validate`` CLI. It is not part of the
...
from bikescore.validation import ValidationReport, compare_dataframes

if TYPE_CHECKING:
    from bikescore.deviations import KnownDeviation
