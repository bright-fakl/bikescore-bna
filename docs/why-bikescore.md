...
`bikescore` is a pure-Python reimplementation of the PeopleForBikes
...
`bikescore` reimplements the same algorithm as an ordinary Python library:
...
At the level of the **algorithm and the numbers**, it does not: `bikescore` targets
...
| | brokenspoke-analyzer | bikescore |
|---|---|---|
| Runtime | PostgreSQL + PostGIS, SQL scripts | Pure Python, in-process |
| Deployment | Docker container + database | `pip install bikescore` |
...
`bikescore` corrects a bug in the reference SQL, makes a different (and better-justified)
...
the ground truth: where the two ever disagree without an entry on that page, `bikescore`
