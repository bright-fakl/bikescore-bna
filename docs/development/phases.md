...
`bikescore-bna` was carved out of `bna-core` in **Phase 38** — the scoring-library /
orchestration split. The core is a clean-slate port, module by module, validated against
`bna-core`'s frozen Aspen output as each piece landed. The A-series below is the core
repo's own history; the parallel B-series (the `bikescore` orchestration layer) lives
in that repo's log.
...
|---|---|
| **38a** | `bna-core` Aspen reference snapshot + `bikescore-bna` scaffold + docs skeleton |
...
| **38f** | End-to-end `score_city` parity — the crux gate: identical Aspen output vs `bna-core` |
| **38g** | `acquire_city` / `InputProvider`, the `bikescore-bna` CLI, release `v0.1.0`, and the core docs |

At **38g** the core became independently installable and validatable: `score_city`
reproduces `bna-core`'s Aspen output with no database, workspace, or web layer, and the
full suite is green on a cold clone.

## After the split

Feature work resumes on the core once it reaches parity. Landed so far:

- **Export** — a database-free export framework (`bikescore_bna.export`) that writes any stage
  ...
  and the `bikescore-bna export` / `export-list` CLI commands. See
